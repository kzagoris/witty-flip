...
function readStoredZipEntryText(buffer: Buffer | Uint8Array, entry: ZipEntry): string | undefined {
...
function isExpectedZipContainer(buffer: Buffer | Uint8Array, ext: string): boolean {
...
export async function validateFile(
    ...
    if (!conversion.sourceExtensions.includes(ext)) {
        return {
            valid: false,
            error: `Invalid file extension "${ext}". Expected: ${conversion.sourceExtensions.join(", ")}`,
        }
    }
