    ...
    <PageShell>
      <div className="py-16 text-center">
        ...
        <p className="mt-2 text-muted-foreground">
          The requested conversion type is not supported.
        </p>
      </div>
    </PageShell>
  ),
...
function ConversionPage() {
  ...
  const syncFileIdInSearch = useCallback((fileId: string | null) => {
    void navigate({
      to: '/$conversionType',
  ...
  const isCanceledReturn = Boolean(canceled && searchFileId)
  const isCheckoutReturn = Boolean(session_id && searchFileId)
  const initialFileId = isCanceledReturn || isCheckoutReturn ? searchFileId : undefined
  const initialState = isCanceledReturn ? 'payment_required' : isCheckoutReturn ? 'converting' : undefined

  const flow = useConversionFlow({
    conversionType: conversion.slug,
    initialFileId,
    initialState,
    initialCanceled: isCanceledReturn,
